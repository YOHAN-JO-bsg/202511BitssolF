...
function VoteSection({ voteOptions, isVotedByUser, onVoteSuccess }: VoteSectionProps) {
    ...
    return (
        <div className="vote-section">
            <div className="vote-section__header">
                ...
                <span className="vote-section__total">총 {totalVotes}표</span>
            </div>
                ...
                {voteOptions.map((option) => {
                    ...
                    return (
                        <div key={option.voteId} className="vote-option">
                            <button
                                className={`vote-option__btn ${option.selectedByUser ? 'selected' : ''} ${hasVoted ? 'voted' : ''
                                    }`}
                                onClick={() => handleVote(option.voteId)}
                                disabled={hasVoted || isLoading}
                            >
                                <div className="vote-option__content">
                                    <span className="vote-option__text">{option.optionText}</span>
                                    {hasVoted && (
                                        <span className="vote-option__result">
                                            {option.voteCount}표 ({percentage}%)
                                        </span>
                                    )}
                                </div>
                                {hasVoted && (
                                    <div className="vote-option__bar" style={{ width: `${percentage}%` }} />
                                )}
                            </button>
                        </div>
                    )
            ...
            {hasVoted && <p className="vote-section__notice">이미 투표하셨습니다.</p>}
        </div>
    )
}
